...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Disposal: %v Social: %v Ethersocial: %v ECIP1017: %v EIP160: %v ECIP1010PauseBlock: %v ECIP1010Length: %v Constantinople: %v ConstantinopleFix: %v Istanbul: %v Muir Glacier: %v Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) IsEIP1884F(num *big.Int) bool {
	return isForked(c.IstanbulBlock, num) && !isForked(c.EIP1884DisableFBlock, num)
}
...
func (c *ChainConfig) IsEIP2384F(num *big.Int) bool {
